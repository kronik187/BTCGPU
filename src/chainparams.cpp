...
public:
    CMainParams() {
        ...
        consensus.BTGVHeight = 491407; // Around 10/25/2017 12:00 UTC
        consensus.BTGVPremineWindow = 8000;
        consensus.BTGVZawyLWMAHeight = 536200; // Around 07/01/2018
        consensus.BTGVEquihashForkHeight = 536200; // Around 07/01/2018
        consensus.BTGVPremineEnforceWhitelist = true;
        ...
        consensus.BTGVMaxFutureBlockTime = 12 * 10 * 60; // 120 mins
...
public:
    CTestNetParams() {
        ...
        consensus.BTGVHeight = 1;
        consensus.BTGVZawyLWMAHeight = -1; // Activated on testnet
        consensus.BTGVEquihashForkHeight = 14300;
        consensus.BTGVPremineWindow = 50;
        consensus.BTGVPremineEnforceWhitelist = false;
        consensus.powLimit = uint256S("0007ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        ...
        consensus.BTGVMaxFutureBlockTime = 7 * 10 * 60; // 70 mins
...
public:
    CRegTestParams() {
        ...
        consensus.BTGVHeight = 2000;
        consensus.BTGVZawyLWMAHeight = -1; // Activated on regtest
        consensus.BTGVEquihashForkHeight = 2001;
        consensus.BTGVPremineWindow = 10;
        consensus.BTGVPremineEnforceWhitelist = false;
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert((uint32_t)consensus.BTGVHeight <= height &&
           height < (uint32_t)(consensus.BTGVHeight + consensus.BTGVPremineWindow));
    int block = height - consensus.BTGVHeight;
    int num_unlocked = consensus.BTGVPremineWindow * 40 / 100;  // 40% unlocked.
    int num_locked = consensus.BTGVPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    if (block < num_unlocked) {
        ...
        int lock_time = consensus.BTGVHeight + stage_lock_time * (1 + stage);
